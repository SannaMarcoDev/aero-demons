...
const MEASURE := 6.0
const ROUNDS := 2
const Sampler = preload("benchmark_sampler.gd")
...
func place_aircraft(loc: Dictionary, t: float) -> void:
	Sampler.place_aircraft(player, camera, loc, t)

func measure(loc: Dictionary, meta: Dictionary, warm: float, meas: float) -> void:
	var result := meta.duplicate()
	result["location"] = loc.id
	result.merge(await Sampler.measure(self, root, player, camera, scene.get_node("GardaLake/Forests"), loc, warm, meas))
	var frame_times: Array = result["frame_times_ms"]
	result.erase("frame_times_ms")
	result.erase("elapsed_ms") # Keep the standalone JSON schema unchanged.
...
static func summarize_frames(frames: Array[float]) -> Dictionary:
	return Sampler.summarize_frames(frames)
