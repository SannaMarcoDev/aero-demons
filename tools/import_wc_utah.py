"""Import Utah's WC Sync export without running WC or the editor bridge.
Requires NumPy and Pillow. Never overwrites an existing destination.
Preserves the export's 4 m spacing in the archived data/manifest; utah_final.tscn
applies the 250 km horizontal extent via Terrain3D.vertex_spacing, without resampling.
For matching vertical proportions, default height-scale is 3.814697265625 (250000 / 65536).
Usage: python tools/import_wc_utah.py --godot PATH_TO_GODOT
"""
import argparse
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--destination', type=Path,
                        default=ROOT / 'terrain/utah_final_wc_uniform_250km')
    parser.add_argument('--height-scale', type=float, default=3.814697265625,
                        help='Explicit vertical import scale around Y=0 (default 250000/65536)')
    ...
    assert tree.find('Project').get('Name') == 'utah', f"Expected project 'utah', got '{tree.find('Project').get('Name')}'"
    ...
    with tempfile.TemporaryDirectory(prefix='wc-utah-') as temp:
        staging = Path(temp)
        for ty in range(4):
            for tx in range(4):
